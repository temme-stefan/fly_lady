...
export default function FilterRegion({values, setValues}
    ...
    const updateUser = (value: string) => {
        const val = (value === "-" ? null : value) as User | null;
        setValues({...values, user: val});
    }
    const updateScope = (value: string) => {
        setValues({...values, scope: value as FilterScope});
    }
    ...
    const userOptions: [string, string][] = Object.entries(User);
    userOptions.unshift(["-", "Alle"]);
    const scopeOptions = Object.entries(FilterScope);
    return (
        <form onSubmit={() => false}>
